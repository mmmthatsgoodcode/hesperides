...
import com.mmmthatsgoodcode.hesperides.datastore.cassandra.astyanax.AstyanaxColumnTransformer;
...
public class AstyanaxCassifierTest {
	...
	public void testCassify() throws TransformationException, SerializationException {
		...
		for(HesperidesRow row:this.rows) {
			...
			Entry<AbstractType, Set<Column<DynamicComposite>>> astyanaxRow = cassifier.transform(row);
//			System.out.println(astyanaxRow);

			// astyanaxRow.getValue(), astyanaxRow.getKey()
			HesperidesRow transformedRow = cassifier.transform(new SimpleEntry<AbstractType, Set<Column<DynamicComposite>>>(astyanaxRow.getKey(), astyanaxRow.getValue()));
